      ...
      <div class="bg-base-200 dark:bg-customGray
        ...
        <div class="flex justify-center gap-30 flex-col h-auto p-40">
          <div class="w-full">
            ...
            <el-input class="h-45 my-10  dark:rounded dark:bg-customGray_shallow" :placeholder="accountPlaceholder"
              v-model="username"></el-input>
          </div>
          <div class="w-full">
            <div class="text-xl mb-5">密码</div>
            <el-input class="h-45 mt-10  dark:rounded dark:bg-customGray_shallow " placeholder="密码" v-model="password"
              type="password"></el-input>
          </div>
          <div class="w-full">
            <div class="text-xl mb-5">重复密码</div>
            <el-input class="h-45 mt-10  dark:rounded dark:bg-customGray_shallow " placeholder="密码" v-model="password"
              @keyup.enter="register" type="password"></el-input>
          </div>
          ...
          <div class="w-full">
            ...
            <el-radio-group v-model="loginType" class="text-center w-full">
              <el-radio :border="true" v-for="user in userTypes" :value=user.value>{{ user.msg }}</el-radio>
            </el-radio-group>
          </div>
        </div>
        <div class="flex justify-evenly items-center mb-20 ">
          <el-button class="dark:opacity-80 shadow-lg hover:-translate-y-1 transform duration-800" size="large"
            type="success" plain @click="register">注册</el-button>
          <el-button class="dark:opacity-80 shadow-lg hover:-translate-y-1 transform duration-800" size="large"
            type="primary" plain @click="back">返回登陆</el-button>
        </div>
      </div>
    </div>
  </div>
...
const accountPlaceholder = computed(() => {
  return userTypes[loginType.value].msg + "账号"
})
const back = () => {
  router.push("/login")
}
